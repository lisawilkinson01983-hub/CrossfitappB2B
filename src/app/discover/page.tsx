...
export default async function DiscoverPage({
  ...
  const [myFollows, myPendingRequests] = await Promise.all([
    prisma.follow.findMany({
      where: { followerId: session.user.id, followingId: { in: resultIds } },
      select: { followingId: true },
    }),
    prisma.followRequest.findMany({
      where: { requesterId: session.user.id, targetId: { in: resultIds }, status: "PENDING" },
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-8">
      <NavBar />
      <h1 className="mt-6 text-2xl font-bold">Discover athletes</h1>
